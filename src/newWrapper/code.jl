...
function MOI.get(model::Optimizer{T}, ::MOI.ConstraintPrimal, c::MOI.ConstraintIndex{MOI.ScalarAffineFunction{T},<:Any},) where T
    #todo
    return zeros(T, 4)
end
...
function MOI.get(model::Optimizer{T}, ::MOI.ConstraintDual, ci::MOI.ConstraintIndex{MOI.VariableIndex,MOI.LessThan{T}}) where T
    return zero(T)
end
function MOI.get(model::Optimizer{T}, ::MOI.ConstraintDual, ci::MOI.ConstraintIndex{MOI.VariableIndex,MOI.EqualTo{T}}) where T
    return zero(T)
end
function MOI.get(model::Optimizer, ::MOI.ConstraintDual, ci::MOI.ConstraintIndex{MOI.VariableIndex,MOI.GreaterThan{T}}) where T
    return zero(T)
end
function MOI.get(model::Optimizer{T}, ::MOI.ConstraintDual, ci::MOI.ConstraintIndex{MOI.ScalarAffineFunction{T},<:Any}) where T
    return zero(T)
end
